...
Додати предмет закупівлі
    ...
    torgibiz.Пошук тендера по ідентифікатору  ${username}  ${tender_uaid}
...
Завантажити документ
    ...
    ...    ${ARGUMENTS[2]} == ${TENDER}
    torgibiz.Пошук тендера по ідентифікатору    ${ARGUMENTS[0]}    ${ARGUMENTS[2]}
...
Оновити сторінку з тендером
    ...
    torgibiz.Пошук тендера по ідентифікатору    ${ARGUMENTS[0]}    ${ARGUMENTS[1]}
...
Отримати інформацію із предмету
    ...
    ${return_value}=    Run Keyword And Return    torgibiz.Отримати інформацію із тендера    ${username}    ${tender_uaid}    ${field_name}
    [Return]    ${return_value}
...
Перейти на сторінку тендера
    ...
    Run Keyword Unless    ${present}    torgibiz.Пошук тендера по ідентифікатору    ${username}    ${tender_uaid}

Отримати інформацію із тендера
    [Arguments]  ${username}  ${tender_uaid}  ${fieldname}
    Run keyword    torgibiz.Перейти на сторінку тендера  ${username}  ${tender_uaid}
    ${return_value}=  Run Keyword  torgibiz.Отримати інформацію про ${fieldname}
    [return]  ${return_value}
...
Отримати інформацію про dgfDecisionDate
    ...
    ${return_value}=    torgibiz_service.convert_date    ${date_value}
    [Return]    ${return_value}
...
Отримати інформацію із запитання
    ...
    torgibiz.Перейти до сторінки запитань    ${username}    ${tender_uaid}
